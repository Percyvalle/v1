...
Движки с тех пор тоже свели в один процесс: модерация переехала внутрь
бота (`cigilbot/pipeline.py`). Панель осталась снаружи по обратной
причине — она ничего не считает и не исполняет, а только пишет в БД то,
что бот оттуда читает. Поэтому её можно уронить или закрыть, и модерация
этого не заметит. Раньше это было не так: supervisor жил в процессе
панели, и её падение останавливало перезапуск упавших консьюмеров.

## Устройство

```
panel/server.py        — сборка приложения: роутеры, сессия
...
`TWITCH_MOD_*` в корневой `.env`, а движок модерации читает их оттуда же. Разъедутся — панель отрапортует об успешно полученном токене, а
