...
подменяется на httpx.MockTransport (тот же приём, что использует
Cigilbot для своей копии этого файла и tests/twitch_api).
"""
...
class TestResolveRole:
    ...
    def test_admin_override_beats_broadcaster_status(self) -> None:
        # admin_override приходит из panel_admins — ручное решение
    ...
    def test_unrecognized_override_value_ignored(self) -> None:
        # Значения вроде "MODERATOR" в panel_admins не должны обходить
...
def auth_app(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[FastAPI, Path]:
    ...
    app.state.panel_db_factory = lambda: Database(str(db))
...
class TestLoginRedirect:
    ...
    def test_503_when_not_configured(self, tmp_path: Path) -> None:
        ...
        app.add_middleware(SessionMiddleware, secret_key="test-secret")
        app.state.panel_auth_config = auth.load_panel_auth_config(tmp_path)
        app.state.panel_db_factory = lambda: Database(str(tmp_path / "bot.db"))
        app.include_router(auth.router)
        client = TestClient(app)
...
class TestCallback:
    ...
    def test_admin_override_from_panel_admins(
        self, auth_app: tuple[FastAPI, Path], monkeypatch: pytest.MonkeyPatch
    ) -> None:
        app, db = auth_app
        ...
        async def seed() -> None:
            await store.connect()
            await store.upsert_panel_admin("viewer1", "ADMIN")
            await store.close()
