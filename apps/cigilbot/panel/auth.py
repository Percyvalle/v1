...
def load_panel_auth_config(
    ...
    env_filename/default_port параметризованы для переносимости кода между
    twitch-bots (panel/server.py, порт 8765, .env) и Cigilbot
    (panel/moderation_server.py, порт 8766, тоже .env — единственный
    процесс с панелью в этом проекте, разносить по разным файлам нечего)."""
    values: dict[str, str] = {}
...
async def _list_profile_channels(root: Path) -> dict[str, str]:
    """{broadcaster_id: channel_login} для каждого канала в Channel Registry
    (registry.db) — источник правды сменился с .env.<profile> файлов на
    Registry (см. docs/master-plan.html, направление 00). Ключ
    словаря по-прежнему называется "profile" в вызывающем коде по
    историческим причинам (см. комментарий в panel/moderation_api.py про
    параметр `profile`), но значением теперь является broadcaster_id.
    Используется для мульти-канальных ролей (см. _resolve_roles_by_channel)
    — один вошедший может быть модератором на одном канале, но не на
    другом, роль не может быть одной строкой на всю сессию."""
    ...
    return {c.broadcaster_id: c.login for c in channels}
...
async def _resolve_roles_by_channel(
    ...
    *,
    ...
    channels = set((await _list_profile_channels(root)).values())
...
async def role_for_profile(request: Request, profile: str) -> str:
    ...
    if not isinstance(roles, dict):
        return "VIEWER"
    root: Path = request.app.state.panel_root
    channel = (await _list_profile_channels(root)).get(profile, "")
    if not channel:
        return "VIEWER"
